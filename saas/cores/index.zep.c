...
PHP_METHOD(Saas_Cores_Index, __construct) {
	...
	if (!(zephir_fast_in_array(pathToKey, &arrLicencePath TSRMLS_CC))) {
		zephir_array_append(&arrLicencePath, pathToKey, PH_SEPARATE, "saas/Cores/Index.zep", 49);
...
PHP_METHOD(Saas_Cores_Index, verifySaasService) {
	...
	if (_6) {
		...
		if (ZEPHIR_IS_EMPTY(&httpCode)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "Saas service is not available!!", "saas/Cores/Index.zep", 93);
			return;
		} else {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "Saas service veirify failed!!", "saas/Cores/Index.zep", 96);
			return;
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Saas_Cores_Index, initial) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zephir_fcall_cache_entry *_0 = NULL, *_11 = NULL, *_19 = NULL;
	zval *pathToKey, pathToKey_sub, __$true, app, serial, serialDecode, moduleName, e, today, expire, today_time, expire_time, _22, _1$$3, _2$$3, _3$$3, _4$$3, _5$$3, _12$$3, _13$$3, _16$$3, _17$$3, _18$$3, _6$$4, _7$$4, _8$$4, _9$$5, _10$$5, _14$$6, _15$$6, _20$$7, _21$$7;
	zval *this_ptr = getThis();
	...
	/* try_start_1: */
		...
		if (zephir_is_true(&_5$$3)) {
			...
			zephir_array_fetch_string(&_8$$4, &_7$$4, SL("licence_id"), PH_NOISY | PH_READONLY, "saas/Cores/Index.zep", 147 TSRMLS_CC);
			ZEPHIR_CALL_METHOD(&_6$$4, this_ptr, "verifylicence", NULL, 18, &_8$$4);
			zephir_check_call_status_or_jump(try_end_1);
			if (!zephir_is_true(&_6$$4)) {
				...
				zephir_check_call_status_or_jump(try_end_1);
				zephir_throw_exception_debug(&_9$$5, "saas/Cores/Index.zep", 148 TSRMLS_CC);
				goto try_end_1;

			}
		}
		zephir_read_property(&_12$$3, &app, SL("licence_config"), PH_NOISY_CC | PH_READONLY);
		zephir_array_fetch_string(&_13$$3, &_12$$3, SL("service_code"), PH_NOISY | PH_READONLY, "saas/Cores/Index.zep", 153 TSRMLS_CC);
		if (!ZEPHIR_IS_EQUAL(&_13$$3, &moduleName)) {
			ZEPHIR_INIT_VAR(&_14$$6);
			object_init_ex(&_14$$6, saas_cores_exceptions_licenceinvalidexception_ce);
			ZEPHIR_INIT_VAR(&_15$$6);
			ZVAL_STRING(&_15$$6, "This licence can not apply to this product");
			ZEPHIR_CALL_METHOD(NULL, &_14$$6, "__construct", &_11, 19, &_15$$6);
			zephir_check_call_status_or_jump(try_end_1);
			zephir_throw_exception_debug(&_14$$6, "saas/Cores/Index.zep", 154 TSRMLS_CC);
			goto try_end_1;

		}
		ZEPHIR_INIT_VAR(&_16$$3);
		ZVAL_STRING(&_16$$3, "Y-m-d");
		ZEPHIR_CALL_FUNCTION(&today, "date", NULL, 20, &_16$$3);
		zephir_check_call_status_or_jump(try_end_1);
		zephir_read_property(&_17$$3, &app, SL("licence_config"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_OBS_VAR(&expire);
		zephir_array_fetch_string(&expire, &_17$$3, SL("expired"), PH_NOISY, "saas/Cores/Index.zep", 158 TSRMLS_CC);
		ZEPHIR_INIT_NVAR(&_16$$3);
		object_init_ex(&_16$$3, php_date_get_date_ce());
		ZEPHIR_CALL_METHOD(NULL, &_16$$3, "__construct", NULL, 0, &expire);
		zephir_check_call_status_or_jump(try_end_1);
		ZEPHIR_INIT_VAR(&_18$$3);
		ZVAL_STRING(&_18$$3, "Y-m-d");
		ZEPHIR_CALL_METHOD(&expire, &_16$$3, "format", NULL, 0, &_18$$3);
		zephir_check_call_status_or_jump(try_end_1);
		ZEPHIR_CALL_FUNCTION(&today_time, "strtotime", &_19, 21, &today);
		zephir_check_call_status_or_jump(try_end_1);
		ZEPHIR_CALL_FUNCTION(&expire_time, "strtotime", &_19, 21, &expire);
		zephir_check_call_status_or_jump(try_end_1);
		if (ZEPHIR_LT(&expire_time, &today_time)) {
			ZEPHIR_INIT_VAR(&_20$$7);
			object_init_ex(&_20$$7, saas_cores_exceptions_licenceexpiredexception_ce);
			zephir_read_property(&_21$$7, &app, SL("licence_config"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_METHOD(NULL, &_20$$7, "__construct", NULL, 22, &_21$$7);
			zephir_check_call_status_or_jump(try_end_1);
			zephir_throw_exception_debug(&_20$$7, "saas/Cores/Index.zep", 166 TSRMLS_CC);
			goto try_end_1;
	...
	if (EG(exception)) {
		...
		Z_ADDREF_P(&_22);
		if (zephir_instance_of_ev(&_22, spl_ce_RuntimeException TSRMLS_CC)) {
			zend_clear_exception(TSRMLS_C);
			ZEPHIR_CPY_WRT(&e, &_22);
			if (zephir_instance_of_ev(&e, saas_cores_exceptions_licenceexception_ce TSRMLS_CC)) {
				zephir_throw_exception_debug(&e, "saas/Cores/Index.zep", 170 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(saas_cores_exceptions_licenceinvalidexception_ce, "Can not parse licence", "saas/Cores/Index.zep", 172);
			return;
		} else {
			if (zephir_instance_of_ev(&_22, zend_exception_get_default(TSRMLS_C) TSRMLS_CC)) {
				zend_clear_exception(TSRMLS_C);
				ZEPHIR_CPY_WRT(&e, &_22);
				if (zephir_instance_of_ev(&e, saas_cores_exceptions_licenceexception_ce TSRMLS_CC)) {
					zephir_throw_exception_debug(&e, "saas/Cores/Index.zep", 170 TSRMLS_CC);
...
PHP_METHOD(Saas_Cores_Index, decrypt) {
	...
	ZEPHIR_CALL_FUNCTION(&key, "hash", NULL, 23, &_5, password, &__$true);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "sha256");
	ZEPHIR_CALL_FUNCTION(&_6, "hash_hmac", NULL, 24, &_5, &ciphertext, &key, &__$true);
	zephir_check_call_status();
	if (!ZEPHIR_IS_IDENTICAL(&_6, &hash)) {
		RETURN_MM_NULL();
	}
	ZVAL_LONG(&_7, 1);
	ZEPHIR_RETURN_CALL_FUNCTION("openssl_decrypt", NULL, 25, &ciphertext, &method, &key, &_7, &iv);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Saas_Cores_Index, getLicenceId) {
	...
	zephir_array_fetch_string(&_4, &config, SL("licence_id"), PH_NOISY | PH_READONLY, "saas/Cores/Index.zep", 253 TSRMLS_CC);
	RETURN_CTOR(&_4);
...
PHP_METHOD(Saas_Cores_Index, getUniqueMachineID) {
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&_0, "WIN")) {
		...
		ZEPHIR_CALL_FUNCTION(&output$$3, "shell_exec", &_10, 28, &_9$$3);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&result, "array_filter", NULL, 29, &lines$$3, &_11$$3);
		zephir_check_call_status();
		if (zephir_fast_count_int(&result TSRMLS_CC) > 0) {
			ZEPHIR_CALL_FUNCTION(&_12$$5, "array_values", NULL, 30, &result);
			zephir_check_call_status();
			ZEPHIR_MAKE_REF(&_12$$5);
			ZEPHIR_CALL_FUNCTION(&result, "array_shift", NULL, 31, &_12$$5);
			ZEPHIR_UNREF(&_12$$5);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(&_14$$5, "end", NULL, 32, &result);
			ZEPHIR_UNREF(&result);
			zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&result, "shell_exec", &_10, 28, &_15$$7);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&_15$$7);
		ZVAL_STRING(&_15$$7, "blkid");
		ZEPHIR_CALL_FUNCTION(&_16$$7, "stripos", NULL, 33, &result, &_15$$7);
		zephir_check_call_status();
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_16$$7)) {
			ZEPHIR_OBS_NVAR(&result);
			zephir_array_fetch_string(&result, _SERVER, SL("HTTP_HOST"), PH_NOISY, "saas/Cores/Index.zep", 279 TSRMLS_CC);
		}
	}
	ZEPHIR_INIT_VAR(&_17);
	...
	ZEPHIR_RETURN_CALL_FUNCTION("sha1", NULL, 34, &_18);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Saas_Cores_Index, randomString) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&bytes, "random_bytes", &_0, 35, &size);
		zephir_check_call_status();
